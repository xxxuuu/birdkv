...
import (
	"birdkv/net"
	"birdkv/persist"
	"birdkv/test-tool/labpersist"
	"birdkv/test-tool/labrpc"
)
import "testing"
import "os"
...
func random_handles(kvh []net.ClientEnd) []net.ClientEnd {
	sa := make([]net.ClientEnd, len(kvh))
	copy(sa, kvh)
	for i := range sa {
		j := rand.Intn(i + 1)
		sa[i], sa[j] = sa[j], sa[i]
...
func (cfg *config) StartServer(i int) {
	...
	if cfg.saved[i] != nil {
		...
		cfg.saved[i] = labpersist.MakeLabPersister()
	}
	cfg.mu.Unlock()
