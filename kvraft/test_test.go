...
import (
	"birdkv/test-tool/models"
	porcupine2 "birdkv/test-tool/porcupine"
)
import "testing"
import "strconv"
import "time"
...
type OpLog struct {
	operations []porcupine2.Operation
	sync.Mutex
}

func (log *OpLog) Append(op porcupine2.Operation) {
	log.Lock()
	defer log.Unlock()
	log.operations = append(log.operations, op)
}

func (log *OpLog) Read() []porcupine2.Operation {
	log.Lock()
	defer log.Unlock()
	ops := make([]porcupine2.Operation, len(log.operations))
...
func Get(cfg *config, ck *Clerk, key string, log *OpLog, cli int) string {
	...
	cfg.op()
	if log != nil {
		log.Append(porcupine2.Operation{
...
func GenericTest(t *testing.T, part string, nclients int, nservers int, unreliable bool, crash bool, partitions bool, maxraftstate int, randomkeys bool) {
	...
	res, info := porcupine2.CheckOperationsVerbose(models.KvModel, opLog.Read(), linearizabilityCheckTimeout)
	if res == porcupine2.Illegal {
		file, err := ioutil.TempFile("", "*.html")
		if err != nil {
			fmt.Printf("info: failed to create temp file for visualization")
		} else {
			err = porcupine2.Visualize(models.KvModel, info, file)
			if err != nil {
	...
	} else if res == porcupine2.Unknown {
